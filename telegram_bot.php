...
if (isset($update['message'])) {
    ...
    if (strpos($text, '/start ') === 0 && !$patient) {
        ...
        if ($patient_id) {
            ...
            $response .= "2. Click 'Open Telegram Bot'\n\n";
    ...
    $booking_step = getBookingStep($chat_id);
    if ($booking_step !== null && $booking_step !== '' && $text !== '/start' && $text !== '/help' && $text !== '/status' && $text !== '/next' && $text !== '/appointments' && $text !== '/queue' && $text !== '/profile' && $text !== '/askappointment') {
            ...
            if (strtolower($text) === 'confirm') {
                ...
                if ($selected_date >= $min_date->format('Y-m-d')) {
                    ...
                    if (count($available_slots) > 0) {
                        ...
                        $response = "❌ No available slots. Please select another date:";
                        setBookingStep($chat_id, 'select_date');
                    }
                    sendMessage($chat_id, $response, $bot_token);
                } else {
