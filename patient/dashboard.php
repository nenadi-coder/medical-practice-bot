...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logout { background: #f56565; padding: 0.5rem 1.2rem; border-radius: 60px; color: white; text-decoration: none; font-weight: 600; font-size: 0.85rem; transition: all 0.2s ease; display: inline-flex; align-items: center; gap: 6px; }
        .logout:hover { background: #e53e3e; transform: translateY(-1px); }
        .container { max-width: 1200px; margin: 2rem auto; padding: 0 1.5rem; }
        .welcome-card { background: white; padding: 1.8rem 2rem; border-radius: 1.5rem; margin-bottom: 2rem; box-shadow: 0 8px 25px -12px rgba(0, 0, 0, 0.1); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; border: 1px solid rgba(102, 126, 234, 0.15); }
        .welcome-card h2 { font-size: 1.5rem; font-weight: 700; color: #1e2a3e; display: flex; align-items: center; gap: 8px; }
        .btn-group { display: flex; gap: 0.75rem; flex-wrap: wrap; }
        .btn { display: inline-flex; align-items: center; gap: 8px; padding: 0.7rem 1.3rem; background: linear-gradient(95deg, #4f46e5, #7c3aed); color: white; text-decoration: none; border-radius: 60px; font-weight: 600; font-size: 0.9rem; transition: all 0.25s ease; border: none; cursor: pointer; }
        .btn:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(79, 70, 229, 0.3); }
        .btn-telegram { background: linear-gradient(95deg, #0088cc, #00a6e6); }
        .btn-telegram:hover { box-shadow: 0 8px 20px rgba(0, 136, 204, 0.3); }
        .btn-cancel { background: #f56565; }
        .btn-cancel:hover { background: #e53e3e; }
        .btn-disabled { background: #cbd5e0; cursor: not-allowed; pointer-events: none; opacity: 0.6; }
        .message { padding: 1rem 1.2rem; border-radius: 1rem; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 10px; font-weight: 500; }
        ...
        .appointments-grid { display: flex; flex-direction: column; gap: 1.25rem; }
        .appointment-card { background: white; padding: 1.5rem; border-radius: 1.25rem; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); border-left: 5px solid #4f46e5; transition: all 0.2s ease; }
        .appointment-card:hover { transform: translateX(4px); box-shadow: 0 8px 25px rgba(0, 0, 0, 0.1); }
        .appointment-card.cancelled { border-left-color: #f56565; opacity: 0.75; background: #fefaf9; }
        .appointment-card.completed { border-left-color: #48bb78; }
        .appointment-header { display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 0.75rem; margin-bottom: 1rem; }
        .doctor-name { font-size: 1.25rem; font-weight: 700; color: #1e2a3e; display: flex; align-items: center; gap: 8px; }
        .status { display: inline-flex; align-items: center; gap: 6px; padding: 0.3rem 0.9rem; border-radius: 60px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
        .status-scheduled { background: #e3f2fd; color: #1976d2; }
        .status-confirmed { background: #e8f5e8; color: #388e3c; }
        .status-completed { background: #f3e5f5; color: #7b1fa2; }
        .status-cancelled { background: #ffebee; color: #c62828; }
        .appointment-details { color: #4a5568; line-height: 1.7; margin-bottom: 1rem; }
        .detail-item { display: flex; align-items: center; gap: 8px; margin: 8px 0; font-size: 0.9rem; }
        .queue-info { background: linear-gradient(135deg, #f0f9ff, #eef2ff); padding: 1rem; border-radius: 1rem; margin: 1rem 0; border: 1px solid rgba(79, 70, 229, 0.2); }
        .queue-position { font-size: 1rem; font-weight: 700; color: #4f46e5; margin: 5px 0; }
        .wait-time { background: #fff3e0; padding: 8px 12px; border-radius: 0.75rem; color: #e67e22; font-weight: 600; margin-top: 8px; }
        .next-message { background: #f0fff4; padding: 8px 12px; border-radius: 0.75rem; color: #48bb78; font-weight: 700; margin-top: 8px; }
        .pending-message { background: #fff8e1; padding: 8px 12px; border-radius: 0.75rem; color: #f59e0b; font-weight: 600; margin-top: 8px; }
        .appointment-actions { display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: 1rem; }
        .empty-state { text-align: center; padding: 3rem; background: white; border-radius: 1.5rem; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.05); }
        .empty-state p { font-size: 1.1rem; color: #5b6e8c; margin-bottom: 1.5rem; }
                    ...
                    ">
                        ...
                        <div class="appointment-details">
                            ...
                            <?php if ($appointment['status'] == 'scheduled' || $appointment['status'] == 'confirmed'): ?>
                                <?php
                                // ✅ DYNAMIC LOGIC: Count by appointment_time instead of static queue_number
                                ...
                                $queue_stmt->execute([
                                    ...
                                    $appointment['appointment_time'],  // ← Dynamic time comparison
                                ...
                                <div class="queue-info">
                                    <!-- ✅ Display calculated position instead of static queue_number -->
                                    <div class="detail-item"><i class="fas fa-ticket-alt"></i> Queue #: <strong><?php echo htmlspecialchars($people_ahead + 1); ?></strong></div>
